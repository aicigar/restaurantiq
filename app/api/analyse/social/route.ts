...
export async function POST(req: NextRequest) {
  ...
  if (!usage.allowed) {
    return json({ error: "Plan limit reached. Please upgrade.", code: "PLAN_LIMIT", used: usage.used, limit: usage.limit, plan: usage.plan }, 403);
  }
  ...
  const readableStream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        if (!fullText.trim()) {
          send({ type: "error", error: "No response from AI. Please try again.", code: "EMPTY_RESPONSE" });
          controller.close();
          return;
        }
        ...
        if (start === -1 || end === -1) {
          send({ type: "error", error: "Failed to parse AI response. Please try again.", code: "PARSE_ERROR" });
          controller.close();
          return;
        }
